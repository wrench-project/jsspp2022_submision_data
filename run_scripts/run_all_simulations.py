...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        ...
        sys.stderr.write("\n    (The workflows directory in the root of this repo contains the workflows ")
    ...
    for platform_config_index in platform_configs:
        ...
        for workflow_json_file in workflow_json_files:
            for alg in algorithms:
                command = simulator + platform + reference_flop + "  --algorithms " + str(
                    alg) + " --workflow " + workflow_json_file
                commands_to_run.append(command)

        with Pool(num_threads) as p:
            p.map(run_simulation, commands_to_run)

        # Our algorithm, alpha = 0.1
            ...
            for workflow in workflow_json_files:
                ...
                for noise in noises:
                    if noise == 0.0:
                        ...
                        seeds = range(1000, 1000 + num_samples)
                    for seed in seeds:
                        command = simulator + platform + reference_flop + scheduler_change_trigger + \
                                  periodic_scheduler_change_trigger + speculative_work_fraction
                        # All algorithms BUT random
                        command += " --workflow " + workflow + " --algorithms 0-" + str(num_algorithms - 2)
                        command += " --simulation_noise " + str(noise) + " --noise_seed " + str(seed)
